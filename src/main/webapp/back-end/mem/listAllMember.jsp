			...
			<table class="table table-bordered text-center table-hover">
				<thead class="table-success">
					<tr>
						...
						<th scope="col" class="col-3">會員帳號</th>
						<!-- 					<th>會員密碼</th> -->
						<!-- 					<th>會員狀態</th> -->
						...
						<th scope="col" class="col-6">修改狀態</th>
						<th>查看</th>
						<th>刪除</th>
					</tr>
				</thead>
				...
				<tbody>
					<c:forEach var="MemVO" items="${list}" begin="<%=pageIndex%>"
						...
						<tr>
							<td style="vertical-align: middle">${MemVO.mbrID}</td>
							<td style="vertical-align: middle">${MemVO.mbrAccount}</td>
							<%-- 						<td>${MemVO.mbrPassword}</td> --%>
							<%-- 						<td>${MemVO.mbrStatus}</td> --%>
							<td style="vertical-align: middle">${MemVO.mbrName}</td>
							<%-- 						<td>${MemVO.mbrGender}</td> --%>
							<%-- 						<td>${MemVO.mbrMobile}</td> --%>
							<%-- 						<td>${MemVO.mbrAddr}</td> --%>
							<%-- 						<td>${MemVO.mbrBirth}</td> --%>
							<td style="vertical-align: middle">${MemVO.mbrEmail}</td>
							<%-- 						<td>${MemVO.mbrJointime}</td> --%>
							<td style="vertical-align: middle">${MemVO.tcoinBal}</td>
							<td class="status">
								<FORM METHOD="post"
									ACTION="<%=request.getContextPath()%>/back-end/mem/mem.do"
									style="margin-bottom: 0px;">

									<select name="mbrStatus">
										<option value="0">未開通</option>
										<option value="1">已開通</option>
										<option value="2">停權</option>

									</select> <input class='delete_ID' type="hidden" name="mbrID"
										value="${MemVO.mbrID}"> <input type="hidden"
										name="action" value="verifyStatusByEMP"> <input
										class='btn btn-warning' type="submit" value="確定">
								</FORM>
							</td>
							<td style="vertical-align: middle">
								<!-- 							<FORM METHOD="post" --> <%-- 								ACTION="<%=request.getContextPath()%>/back-end/mem/mem.do" --%>
								<!-- 								style="margin-bottom: 0px;"> --> <!-- 								<input type="submit" value="修改">  -->
								<%-- 								<input type="hidden" name="mbrID" value="${MemVO.memID}">  --%>
								<!-- 									<input type="hidden" name="action" value="getOne_For_Update"> -->
								<!-- 							</FORM> -->
								<div class="wrap">
									<a id="detail" class="btn popup-btn "
										href="#letmeopen${MemVO.mbrID}"><i class="fas fa-eye"></i></a>
								</div>
								<div class="popup-wrap " id="letmeopen${MemVO.mbrID}">
									<div class="popup-box transform-out">
										...
										<h3 class="shipState">會員狀態：</h3>
										<h4 class="shipState">${MemVO.mbrStatus}</h4>
										<h3>
											會員姓名：${MemVO.mbrName}<br>
										</h3>
										<h3>
											會員性別：${MemVO.mbrGender eq 0?"男":"女"}<br>
										</h3>
										<h3>
							...
							<td   style="vertical-align: middle">
								<FORM class="status" METHOD="post"
									...
									<input class='delete_ID' type="hidden" name="mbrID"
										...
										class='btn btn-danger' onclick="showalert()" type="submit"
										value="刪除">
								</FORM>
							</td>
						</tr>
